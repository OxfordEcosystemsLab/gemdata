...
#  2   X  cwd_transect_num     numeric, x >= 0
#  3   X  cwd_num              numeric, x >= 0  
...
# 
class EgmCWDRespirationImporter < EgmRespirationImporter
  ...
  def read_row(values, logger)
    @values = values
    @rec = find_or_new({
      respiration_value_type: EgmRespirationValue::Type::CWD,
      plot:                   plot,
